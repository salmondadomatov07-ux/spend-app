...
if (!list.length) {
  ...
  try {
    const USER_ID = window.Telegram?.WebApp?.initDataUnsafe?.user?.id || 5522700870;
    const res = await fetch(`http://127.0.0.1:8000/data/${USER_ID}`);
    ...
    const givenLoanEl = document.getElementById("givenLoanValue");
    const takenLoanEl = document.getElementById("takenLoanValue");

    if (givenLoanEl) givenLoanEl.textContent = "0";
    if (takenLoanEl) takenLoanEl.textContent = "0";
...
async function addTransaction() {
  ...
  try {
    ...
    const noteInput = document.getElementById("txName");

   // typeSelect yo‘q — default qilamiz
    const tx_type = "expense";

    const amountText = (amountInput?.value || "").trim();
const amount = Number(amountText);
const note = (noteInput?.value || "").trim();
const tx_type = typeSelect?.value || "expense";
...
window.addEventListener("load", loadDashboardFromAPI);